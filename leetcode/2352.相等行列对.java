...
class Solution {
    public int equalPairs(int[][] grid) {
        ...
        for(int i=0;i<n;i++){
            String key="";
            for(int j=0;j<n;j++){
               key+=String.valueOf(grid[j][i])+"_";
            }
            rolMap.put(key, rolMap.getOrDefault(key, 0)+1);
        }
        for(String key: colMap.keySet()){
            if(rolMap.containsKey(key)){
                count+=colMap.get(key)*rolMap.get(key);
            }
        }
        return count;
    }
